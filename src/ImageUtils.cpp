...
ofImage * ift3100::ImageUtils::importImage(const std::string& path) {
    ofLog() << "<utils:import> from " << path;
    return new ofImage(path);
}

void ift3100::ImageUtils::exportImage(const std::string& name) {
 ...
 */
unsigned int ** ift3100::ImageUtils::computeHistRGB(const ofImage &image) {
 ...
 */
ofImage ift3100::ImageUtils::convert(const ofImage& image, int code) {
